...
const UpdateManager = () => {
  ...
  const formattedReleaseNotes = formatReleaseNotes(
...
const isAutoGeneratedGithubNote = (text) => {
  if (!text) {
    return false;
  }
...
const sanitizeReleaseNote = (note) => {
  ...
  const cleaned = (summaryOnly || decoded).trim();

  if (isAutoGeneratedGithubNote(cleaned)) {
    return '';
  }
...
const extractReleaseName = (note) => {
  if (!note) {
    return '';
  }

  if (typeof note === 'object') {
    return note.releaseName || note.name || note.title || '';
  }

  return '';
};

const formatReleaseNotes = (releaseNotes, fallbackReleaseName = '') => {
  if (!releaseNotes) {
    return fallbackReleaseName || '';
  }
  ...
  const sanitizedNotes = notesArray
    .map(note => ({
      text: sanitizeReleaseNote(note),
      releaseName: extractReleaseName(note)
    }))
    .map(({ text, releaseName }) => {
      if (text) {
        return text;
      }
